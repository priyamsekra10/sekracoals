...
export default function Product({
  ...
  return (
    ...
    >
      ...
      >
        {product.new && (
          <span className="mb-[1.5rem] block text-3xl uppercase leading-normal text-orange md:mb-[1rem] xl:mb-[1rem]">
            {product.title}
          </span>
        )}
        <h2
          className={twJoin(
            "max-w-[40ch] text-1xl font-bold uppercase text-neutral-900  xl:mx-0",
            !showAddToCart && "mx-auto",
          )}
        >
          {product.name}
        </h2>
        <p className="mt-[2rem] text-base opacity-50 md:mt-[1rem]">
          {product.description}
        </p>
        {showAddToCart ? (
          <>
